...
module "ide_utilities" {
  source           = "git::https://github.com/abridgeai/coder.git//modules/utilities/ide?ref=v1.8.7"
  start_count      = data.coder_workspace.me.start_count
  agent_id         = coder_agent.main.id
...
module "logger" {
  source = "git::https://github.com/abridgeai/coder.git//modules/logger?ref=v1.8.7"
}
...
locals {
  ...
  # Repository configuration - hardcoded to clinician-web
  repo_url  = "https://github.com/abridgeai/clinician-web"
  repo_name = "clinician-web"
...
# --- Coder Application: SOAP Dashboard ---
resource "coder_app" "soap-dashboard" {
  agent_id     = coder_agent.main.id
  slug         = "soap-dashboard"
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          # SOAP Dashboard secrets from coder-soap-dashboard-secrets
          env {
            name = "NPM_TOKEN"
            value_from {
              secret_key_ref {
                name = "coder-soap-dashboard-secrets"
                key  = "NPM_TOKEN"
              }
            }
          }
