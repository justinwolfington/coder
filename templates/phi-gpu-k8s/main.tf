...
module "cpu_resources" {
  source = "git::https://github.com/abridgeai/coder.git//modules/resources/cpu?ref=v1.13.0"
}

module "gpu_resources" {
  source = "git::https://github.com/abridgeai/coder.git//modules/resources/gpu?ref=v1.13.0"
}

module "git_utilities" {
  source       = "git::https://github.com/abridgeai/coder.git//modules/utilities/git?ref=v1.13.0"
...
module "workspace_scripts" {
  source       = "git::https://github.com/abridgeai/coder.git//modules/utilities/workspace-scripts?ref=v1.13.0"
  start_count  = data.coder_workspace.me.start_count
  agent_id     = coder_agent.main.id
...
# NOTE: PHI egress is locked to an allowlist and the filesystem is read-only
# except for the home PVC (/root) and /tmp. A user's dotfiles repo host must be
# on the PHI egress allowlist or `coder dotfiles` will fail to clone. An empty
# Dotfiles URL is a no-op and is safe.
module "dotfiles" {
...
module "utd_bucket" {
  source                 = "git::https://github.com/abridgeai/coder.git//modules/utilities/gcs-bucket?ref=v1.13.0"
...
locals {
  ...
  # Kubernetes metadata - Updated for PHI workspaces
  labels = {
  ...
  annotations = {
    ...
    "com.coder.workspace.phi-enabled" = "true"
  }
...
resource "coder_agent" "main" {
  ...
  # PHI Security: Disable all insecure display apps
  display_apps {
    port_forwarding_helper = false
    ssh_helper             = false
    vscode                 = false # Disable desktop VSCode
    vscode_insiders        = false # Disable VSCode Insiders
    web_terminal           = false # Disable web terminal
  }
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          # PHI-specific environment variables
          env {
            name  = "PHI_WORKSPACE"
            value = "true"
          }
